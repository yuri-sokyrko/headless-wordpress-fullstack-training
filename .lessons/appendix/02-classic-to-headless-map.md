...
| `'tax_query' => [...]` | `incidents(where: { taxQuery: ... })` | — |
| `'meta_query' => [...]` | `where: { metaQuery: ... }` | Still slow for the same reason it was always slow — `wp_postmeta.meta_value` has no index. Headless does not fix this; it just hides it one layer further away. See Lesson 02.3. |
...
| `get_option('some_setting')` | `siteSettings { … }` via an ACF options page | — |
