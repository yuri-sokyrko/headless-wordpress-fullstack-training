...
values are `SCREAMING_SNAKE_CASE` by convention, the underlying SCF select values are
...
- SCF select fields resolving through those enums, with `kebab-case` mapped to
...
registered everything it derives from your post types, taxonomies and SCF groups — so your
...
Right now `incidentDetails { environment }` is a `String`, because that is what SCF stores and
WPGraphQL for SCF has no way to know the value set is closed. Everything downstream inherits
that shrug.
...
GraphQL enum values are `SCREAMING_SNAKE_CASE` by convention. The SCF select values in
...
WPGraphQL for SCF has already registered them as `String`. You cannot register a field twice, so
...
| Cost | Detail |
|---|---|
| You now own the resolver | SCF's resolver handled `return_format`, translation and the field's default. Yours reads meta directly, so anything SCF did for you, you do or lose. |
| The type name is not yours | `IncidentDetails` is generated by WPGraphQL for SCF from `graphql_field_name`. A major version of that plugin could rename it, and your `deregister` would then silently target nothing. |
...
| `get_field( 'downtime_minutes', $id )` | ⚠️ SCF's own load path, per node | Lesson 06.4 measures it |
...
### Step 4: Find out what the SCF field-group type is really called

The next step deregisters two fields on the type WPGraphQL for SCF generated from the
...
- [ ] The output is a type name — `IncidentDetails` if you are on WPGraphQL for SCF v2.
- [ ] If it differs, note it. Step 5 puts it in a single constant, `SCF_INCIDENT_TYPE`, and that
...
function register_incident_enum_fields(): void {
	...
	foreach ( $fields as $field_name => list( $enum, $meta_key ) ) {
		// SCF registered this field as String. A field cannot be registered
...
# 4. The SCF field is now enum-typed, not String
curl -s -X POST http://localhost:8080/graphql -H 'Content-Type: application/json' \
